...
- LangGraph 与五个智能体实时执行轨迹已在前端展示。
- 任务冻结模型、规则集、协议及四个 Dify Workflow 版本。
- 人工复核提交记录已形成可追溯审计轨迹。
- 同主体的联系信息、设备网络、文件元数据和文本雷同线索可归并为多信号组合异常。
- 长文档视觉检测默认不再截断前 100 页；资源受限环境可通过配置重新设置上限。
- 企业级 Demo 已提供自动评测脚本，统计预期发现召回率、证据覆盖率和人工复核覆盖率。
