...
人工复核暂停期间仍可查看解析概览、关键字段、智能体阶段结果、显式解析计划、
质量门重试记录和标准证据切片。项目证据检索采用任务隔离索引，支持文件、页码和
内容类型过滤；法规依据仍由 Dify/RAG 长期知识库负责，两者职责互不混淆。
...
完成 Demo 任务后，可以运行 `scripts/evaluate_enterprise_demo.py`，自动计算预期发现
召回率、证据覆盖率与人工复核覆盖率。任务同时冻结模型、规则集和各 Dify Workflow
版本，并保存人工复核审计记录，便于验收与争议追溯。
